...
The importance of this equation is related to the study of water waves in presence of capillarity.\pause
    ...
    \end{block}\pause
        ...
        equation} or \structure{extended Fisher-Kolmogorov
...
\begin{frame}{About spectral stability}
    \begin{block}{Solitary waves as homoclinic solutions}
        ...
        their steady part is a \structure{finite-dimensional Hamiltonian
    ...
    \begin{block}{Advantage of the Hamiltonian structure}
        Linear and nonlinear Hamiltonian systems have \structure{global
    ...
    \begin{enumerate}
    \item \structure{Maslov index}
    \item \structure{Evans function}
    \end{enumerate}
    \end{block}
    \end{frame}
...
\begin{frame}{About the existence}
    \begin{block}{Homoclinic solutions}
        ...
        \structure{mountain-pass procedure} for the associated action functional.
    \end{block}
    \pause
...
\begin{frame}{About the forcing theory}
    \begin{block}{The Swift-Hohenberg Lagrangian}\pause
        is an example of \structure{twist} \pause and \structure{dissipative
        system.} \pause It is possible to construct a
        Morse-type theory on certain spaces of braid diagrams.\pause

        We define a \structure{topological invariant} of closed positive braids which
        is correlated with the existence of invariant sets of \structure{parabolic
...
\begin{frame}{Conclusion}
    \begin{block}{Evolution of the complexity} % TODO: what does the next sentence mean?
...
\begin{frame}{Braid diagrams }
    \begin{block}{Labeled projection}\pause
        ...
        a \structure{braid diagram}.\pause
    ...
    \end{block}
        ...
        crossing is \structure{bottom over top} or \structure{top over bottom}
        respectively.\pause

        \structure{Since we are dealing with Legendrian braids all crossings have
        positive labels.}
        \end{notation}
\end{frame}
...
\begin{frame}{Topological transversality}
        \begin{block}{Definition}
            ...
            \structure{closed positive braid diagrams on $n$ strands pairwise
...
\begin{frame}{Discretized braid diagrams}
    \begin{block}{Definition}
        ...
        \R^{d+1}$;\pause
        \item \structure{Periodicity:} $u_d^\alpha= u_0^{\tau(\alpha)}$;\pause
        \item \structure{Transversality:} for any $\alpha\neq \alpha'$ such
...
\begin{frame}{Admissibility and length}\pause
    \begin{block}{Braiding data are lost if the discretization is too coarse}
            A discretization period $d$ is \structure{admissible} for
            $\vu \in \Omega^n$ if
            \[
            \{PL(DISC(\vu))\}=\{\vu\}.
            \]
        ...
        \begin{block}{The length}
            The \structure{length} of a topological braid $\vu\in \Omega^n$, denoted by
...
\begin{frame}{Singular discretized braids}
    ...
    For \structure{topological braids}, a \alert{singular braid} arises
    when any strands intersect. \pause

For a \structure{discretized braid}
...
\begin{frame}{Braid-theoretic comparison principle}
    \begin{block}{Parabolic flow}
    Given a discretized braid $\vu=\{u_i^\alpha\}$ and a parabolic
    relation $\mathcal R$, one evolves braid according to the
    equation
    \[
    \dfrac{d}{dt}(u_i^\alpha)= \mathcal R_i(u_{i-1}^\alpha, u_i^\alpha,
    u_{i+1}^\alpha).
    \]
    \end{block}\pause
    \begin{block}{Stationary solutions}
    corresponds to the braids $\vu$ such that $\mathcal R_i(\vu)=0$
    for all $i$. \pause

    \structure{The parabolic relation $\mathcal R$ induces a flow on $\mathcal D_d^n$ which
...
\begin{frame}{A crucial result}\pause
    \begin{block}{The algebraic length is a Lyapunov function}
        Let $\mathcal R$ be any parabolic relation and $\vu \in \Sigma$
        any singular braid. Then the flowline $\vu(t)$ of $\mathcal R$
    ...
    \end{block}
        ...
        \includegraphics[width=0.5\textwidth]{images/Fig112Wojcik2.pdf}\caption{Along the evolution of the parabolic flow crossing can be
...
\begin{frame}{The index}
    \begin{block}{Relative discretized braids}
        Given a period $d$ braid $\vv$, we denote by $\mathcal D_d^n
        rel\vv$ the space of all $n$ strands, period $d$ discretized braid
        which have $\vv$ has sub-braid.
        \end{block}\pause
    \begin{block}{Properness and boundedness}
        The discretized relative braid class $[\vu rel \vv]$ is \alert{proper}
        if no free strands of $\vu$ can collapse onto $\vv$ or onto each
        other.\pause \\
        It is \alert{bounded} if there exists a uniform bound on all
...
\begin{figure}\label{fig:improper_Bounded}
        \includegraphics[width=0.7\textwidth]{images/Fig4Inventiones22.pdf}\caption{Improper and bounded relative braid class.}
        \end{figure}
\end{frame}
\begin{frame}{...One more picture}
\begin{figure}\label{fig:proper_Bounded}
        \includegraphics[width=0.7\textwidth]{images/Fig4Inventiones21.pdf}\caption{Proper and bounded relative braid class.}
        \end{figure}
\end{frame}
...
\begin{frame}{Topological content}
    ...
    which arise as stationary solutions of $\mathcal R$.
    \end{enumerate}
    \end{thm}
\end{frame}
\begin{frame}{Applications}
    \begin{block}{Second order Lagrangians}
        ...
        \]
    \end{block}\pause
    \begin{block}{How to find periodic orbits?}
